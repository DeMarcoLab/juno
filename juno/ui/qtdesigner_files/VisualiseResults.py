# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'VisualiseResults.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(536, 1048)
        MainWindow.setMaximumSize(QtCore.QSize(16777215, 16777215))
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.gridLayout = QtWidgets.QGridLayout(self.centralwidget)
        self.gridLayout.setObjectName("gridLayout")
        self.frame_selection = QtWidgets.QFrame(self.centralwidget)
        self.frame_selection.setMaximumSize(QtCore.QSize(16777215, 16777215))
        self.frame_selection.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_selection.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_selection.setObjectName("frame_selection")
        self.gridLayout_2 = QtWidgets.QGridLayout(self.frame_selection)
        self.gridLayout_2.setObjectName("gridLayout_2")
        spacerItem = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.gridLayout_2.addItem(spacerItem, 13, 0, 1, 1)
        self.gridLayout_3 = QtWidgets.QGridLayout()
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.doubleSpinBox_scale = QtWidgets.QDoubleSpinBox(self.frame_selection)
        self.doubleSpinBox_scale.setMinimum(0.0)
        self.doubleSpinBox_scale.setMaximum(1000.0)
        self.doubleSpinBox_scale.setSingleStep(0.001)
        self.doubleSpinBox_scale.setProperty("value", 1.0)
        self.doubleSpinBox_scale.setObjectName("doubleSpinBox_scale")
        self.gridLayout_3.addWidget(self.doubleSpinBox_scale, 3, 1, 1, 1)
        self.checkBox_log_plots = QtWidgets.QCheckBox(self.frame_selection)
        self.checkBox_log_plots.setObjectName("checkBox_log_plots")
        self.gridLayout_3.addWidget(self.checkBox_log_plots, 2, 0, 1, 1)
        self.label_vis_scale = QtWidgets.QLabel(self.frame_selection)
        self.label_vis_scale.setObjectName("label_vis_scale")
        self.gridLayout_3.addWidget(self.label_vis_scale, 3, 0, 1, 1)
        self.label_vis_header = QtWidgets.QLabel(self.frame_selection)
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.label_vis_header.setFont(font)
        self.label_vis_header.setObjectName("label_vis_header")
        self.gridLayout_3.addWidget(self.label_vis_header, 1, 0, 1, 1)
        self.checkBox_show_all_simulations = QtWidgets.QCheckBox(self.frame_selection)
        self.checkBox_show_all_simulations.setChecked(True)
        self.checkBox_show_all_simulations.setObjectName("checkBox_show_all_simulations")
        self.gridLayout_3.addWidget(self.checkBox_show_all_simulations, 2, 1, 1, 1)
        self.gridLayout_2.addLayout(self.gridLayout_3, 8, 0, 1, 1)
        self.frame_filter = QtWidgets.QFrame(self.frame_selection)
        self.frame_filter.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_filter.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_filter.setObjectName("frame_filter")
        self.gridLayout_4 = QtWidgets.QGridLayout(self.frame_filter)
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.label = QtWidgets.QLabel(self.frame_filter)
        self.label.setObjectName("label")
        self.gridLayout_4.addWidget(self.label, 0, 0, 1, 1)
        self.label_show_columns = QtWidgets.QLabel(self.frame_filter)
        self.label_show_columns.setObjectName("label_show_columns")
        self.gridLayout_4.addWidget(self.label_show_columns, 3, 0, 1, 1)
        self.spinBox_num_filters = QtWidgets.QSpinBox(self.frame_filter)
        self.spinBox_num_filters.setMinimum(1)
        self.spinBox_num_filters.setMaximum(10)
        self.spinBox_num_filters.setObjectName("spinBox_num_filters")
        self.gridLayout_4.addWidget(self.spinBox_num_filters, 0, 2, 1, 1)
        self.lineEdit_show_columns = QtWidgets.QLineEdit(self.frame_filter)
        self.lineEdit_show_columns.setObjectName("lineEdit_show_columns")
        self.gridLayout_4.addWidget(self.lineEdit_show_columns, 3, 2, 1, 1)
        self.label_num_filtered_simulations = QtWidgets.QLabel(self.frame_filter)
        self.label_num_filtered_simulations.setObjectName("label_num_filtered_simulations")
        self.gridLayout_4.addWidget(self.label_num_filtered_simulations, 7, 0, 1, 3)
        self.scroll_area_filter = QtWidgets.QScrollArea(self.frame_filter)
        self.scroll_area_filter.setMinimumSize(QtCore.QSize(0, 0))
        self.scroll_area_filter.setMaximumSize(QtCore.QSize(16777215, 500))
        self.scroll_area_filter.setWidgetResizable(True)
        self.scroll_area_filter.setObjectName("scroll_area_filter")
        self.scrollAreaWidgetContents_2 = QtWidgets.QWidget()
        self.scrollAreaWidgetContents_2.setGeometry(QtCore.QRect(0, 0, 476, 232))
        self.scrollAreaWidgetContents_2.setObjectName("scrollAreaWidgetContents_2")
        self.scroll_area_filter.setWidget(self.scrollAreaWidgetContents_2)
        self.gridLayout_4.addWidget(self.scroll_area_filter, 2, 0, 1, 3)
        self.pushButton_reset_data = QtWidgets.QPushButton(self.frame_filter)
        self.pushButton_reset_data.setObjectName("pushButton_reset_data")
        self.gridLayout_4.addWidget(self.pushButton_reset_data, 4, 0, 1, 1)
        self.pushButton_filter_data = QtWidgets.QPushButton(self.frame_filter)
        self.pushButton_filter_data.setObjectName("pushButton_filter_data")
        self.gridLayout_4.addWidget(self.pushButton_filter_data, 4, 2, 1, 1)
        self.gridLayout_2.addWidget(self.frame_filter, 5, 0, 1, 1)
        self.comboBox_napari_sim = QtWidgets.QComboBox(self.frame_selection)
        self.comboBox_napari_sim.setObjectName("comboBox_napari_sim")
        self.gridLayout_2.addWidget(self.comboBox_napari_sim, 9, 0, 1, 1)
        self.label_sim_no_loaded = QtWidgets.QLabel(self.frame_selection)
        self.label_sim_no_loaded.setText("")
        self.label_sim_no_loaded.setObjectName("label_sim_no_loaded")
        self.gridLayout_2.addWidget(self.label_sim_no_loaded, 3, 0, 1, 1)
        self.pushButton_update_visualisation = QtWidgets.QPushButton(self.frame_selection)
        self.pushButton_update_visualisation.setObjectName("pushButton_update_visualisation")
        self.gridLayout_2.addWidget(self.pushButton_update_visualisation, 10, 0, 1, 1)
        self.pushButton_load_simulation = QtWidgets.QPushButton(self.frame_selection)
        self.pushButton_load_simulation.setObjectName("pushButton_load_simulation")
        self.gridLayout_2.addWidget(self.pushButton_load_simulation, 2, 0, 1, 1)
        self.tableLayout = QtWidgets.QVBoxLayout()
        self.tableLayout.setObjectName("tableLayout")
        self.gridLayout_2.addLayout(self.tableLayout, 7, 0, 1, 1)
        self.label_sim_run_name = QtWidgets.QLabel(self.frame_selection)
        self.label_sim_run_name.setObjectName("label_sim_run_name")
        self.gridLayout_2.addWidget(self.label_sim_run_name, 1, 0, 1, 1)
        self.label_filter_title = QtWidgets.QLabel(self.frame_selection)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_filter_title.setFont(font)
        self.label_filter_title.setObjectName("label_filter_title")
        self.gridLayout_2.addWidget(self.label_filter_title, 4, 0, 1, 1)
        self.label_title = QtWidgets.QLabel(self.frame_selection)
        font = QtGui.QFont()
        font.setPointSize(16)
        font.setBold(True)
        font.setWeight(75)
        self.label_title.setFont(font)
        self.label_title.setObjectName("label_title")
        self.gridLayout_2.addWidget(self.label_title, 0, 0, 1, 1)
        self.gridLayout.addWidget(self.frame_selection, 0, 0, 1, 1)
        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 536, 21))
        self.menubar.setObjectName("menubar")
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)

        self.retranslateUi(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)
        MainWindow.setTabOrder(self.pushButton_load_simulation, self.spinBox_num_filters)
        MainWindow.setTabOrder(self.spinBox_num_filters, self.scroll_area_filter)
        MainWindow.setTabOrder(self.scroll_area_filter, self.lineEdit_show_columns)
        MainWindow.setTabOrder(self.lineEdit_show_columns, self.pushButton_reset_data)
        MainWindow.setTabOrder(self.pushButton_reset_data, self.pushButton_filter_data)
        MainWindow.setTabOrder(self.pushButton_filter_data, self.checkBox_log_plots)
        MainWindow.setTabOrder(self.checkBox_log_plots, self.checkBox_show_all_simulations)
        MainWindow.setTabOrder(self.checkBox_show_all_simulations, self.doubleSpinBox_scale)
        MainWindow.setTabOrder(self.doubleSpinBox_scale, self.comboBox_napari_sim)
        MainWindow.setTabOrder(self.comboBox_napari_sim, self.pushButton_update_visualisation)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.checkBox_log_plots.setText(_translate("MainWindow", "Logarithmic Plots"))
        self.label_vis_scale.setText(_translate("MainWindow", "Visualisation Scale (Z-Axis)"))
        self.label_vis_header.setText(_translate("MainWindow", "Visualisation Options"))
        self.checkBox_show_all_simulations.setText(_translate("MainWindow", "Show All Simulations"))
        self.label.setText(_translate("MainWindow", "Number of filters"))
        self.label_show_columns.setText(_translate("MainWindow", "Show columns"))
        self.label_num_filtered_simulations.setText(_translate("MainWindow", "TextLabel"))
        self.pushButton_reset_data.setText(_translate("MainWindow", "Reset Data"))
        self.pushButton_filter_data.setText(_translate("MainWindow", "Filter Data"))
        self.pushButton_update_visualisation.setText(_translate("MainWindow", "Update Visualisation"))
        self.pushButton_load_simulation.setText(_translate("MainWindow", "Load Simulation"))
        self.label_sim_run_name.setText(_translate("MainWindow", "No simulation loaded."))
        self.label_filter_title.setText(_translate("MainWindow", "Filter Results"))
        self.label_title.setText(_translate("MainWindow", "Simulation Results"))

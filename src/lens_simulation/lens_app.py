...
with st.form("lens_form"):
    ...
    grating_width_m = form_cols[2].number_input("Grating Width (um)", min_value=0.0, max_value=1000.0, value=1.0, step=0.1) * MICRON_TO_METRE
    grating_distance_m = form_cols[2].number_input("Grating Distance (um)", min_value=0.0, max_value=diameter * METRE_TO_MICRON , value=10.0, step=0.1) * MICRON_TO_METRE
    grating_depth_m = form_cols[2].number_input("Grating Depth (um)", min_value=0.0, max_value=height * METRE_TO_MICRON , value=1.0, step=0.1) * MICRON_TO_METRE
...
if submitted:
    ...
    lens.generate_profile(1e-6, lens_type=LensType.Spherical)
    lens.calculate_grating_mask(grating_width_m=grating_width_m, distance_m=grating_distance_m, depth_m=grating_depth_m)
